...
import { Container, Card, Button, Form, Navbar, Nav } from 'react-bootstrap';
...
const getRandomQuestions = (data) => {
  const shuffled = [...data].sort(() => Math.random() - 0.5); // Shuffle the questions
  return shuffled.slice(0, 10); // Return 10 random questions
};
...
function Quiz() {
  ...
  // Fetch 10 random questions on component mount
  useEffect(() => {
    setQuestions(getRandomQuestions(quizData)); // Set 10 random questions
  }, []);
  ...
  const handleRestart = () => {
    setQuestions(getRandomQuestions(quizData)); // Restart with new random questions
  ...
  return (
    <div className='bg-light'>
      <Navbar bg="dark" variant="dark" expand="lg">
        <Navbar.Brand href="#home" className="mx-4" style={{ color: 'white' }}>
          QuizHunt
        </Navbar.Brand>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="mr-auto">
            ...
            <Nav.Link href="#quizzes" style={{ color: 'white' }}>Quizzes</Nav.Link>
          </Nav>
        </Navbar.Collapse>
      </Navbar>
        ...
        {showScore ? (
          ...
          <Card style={{ color: 'black' }}>
            <Card.Body>
              ...
              {/* Change color of question to black explicitly */}
              <Card.Text style={{ color: 'black' }}>{questions[currentQuestion].question}</Card.Text>
              <Form>
                {['a', 'b', 'c', 'd'].map((option) => (
                  <Form.Check
                    key={option}
                    type="radio"
                    label={questions[currentQuestion][option]}
                    name="quizOptions"
                    value={option}
                    checked={selectedAnswer === option}
                    onChange={handleAnswerChange}
                    style={{ color: 'black' }} // Text color changed to black
                  />
                ))}
              </Form>
              <Button
